...
$(window).bind("load",function() {
	...
	var cistome = querystring('cistome').toString();	// Cistome Mode
	...
	// Load an AGI from CNS data
	if ((agi !== "") && (dataSource === "CNSData")) {
	...
	if ((agi == "") && (dataSource == "CNSData") && (start != "") && (end != "") && (chr != "")) {
		if (start <= 0) {
			alert("Start location should be at least 1.");
			location.href = "http://bar.utoronto.ca/geneslider/";
		}
		alignmentLoader(parseInt(chr, 10), parseInt(start, 10), parseInt(end, 10), parseInt(zoomFrom, 10), parseInt(zoomTo, 10), weightedBitscore, alnIndicator, cistome);
		$('#topPageText').hide();
		createButton('Download Alignment');
		createButton('Download Sequences');
		return;
